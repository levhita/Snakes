...
#define grownpergreen 50
//Starting Grown >=1
#define grownpoints   50
...
void finish(){
	...
	setcolor(WHITE);
	cleardevice();
	outtextxy(getmaxx()-textheight(""),getmaxy(),"Thanks For Playing");
	cursor(getmaxx()-textheight(""),getmaxy()-(textwidth("Thanks For Playing")));
	closegraph();
	exit(1);
}
...
{
	 //Set VGA640x480x16
	 int gdriver = driver, gmode=mode, errorcode;
...
void player::initall(int n){
	if(n==1){
		number=1;
		initialize();
		color[0]=LIGHTMAGENTA;
		color[1]=LIGHTBLUE;
		color[2]=LIGHTCYAN;
		color[3]=LIGHTMAGENTA;
		color[4]=LIGHTBLUE;
		lives=vidas;
